...
describe('Reward Distributor Contract Testing', () => {
  ...
  function checkCanHarvest(state: ContractState<RewardDistributorTypes.Fields>, timestamp: bigint) {
    return RewardDistributor.tests.canHarvest({
      initialFields: state.fields,
      initialAsset: state.asset,
  ...
  describe('Harvest Functionality Tests', () => {
    test('Harvest 2 Epochs After Adding Rewards With Multiple Harvests', async () => {
      ...
      for (var i = 1n; i <= 2; i++) {
        for (var j = 1n; j <= 5; j++) {
          const canHarvest = await checkCanHarvest(rewardDistState, BigInt(genesis) + (epochDuration * i) + (((epochDuration / 5n) * j) - 1n));
          expect(canHarvest.returns).toBe(true);
    ...
    test('Harvest All Epochs After Adding Rewards And Adding Another Reward', async () => {
      ...
      for (var i = 1n; i <= 14; i++) {
        ...
        const canHarvest = await checkCanHarvest(rewardDistState, BigInt(genesis) + (epochDuration * i) + (epochDuration - 1n));
        expect(canHarvest.returns).toBe(true);
    ...
    test('Harvest At Mid Second Epoch After Adding Rewards', async () => {
      ...
      const canHarvest = await checkCanHarvest(rewardDistState, BigInt(genesis) + epochDuration + epochDuration / 2n);
      expect(canHarvest.returns).toBe(true);
    ...
    test('Harvest At 1/10 Second Epoch After Adding Rewards', async () => {
      ...
      const canHarvest = await checkCanHarvest(rewardDistState, BigInt(genesis) + epochDuration + epochDuration / 10n);
      expect(canHarvest.returns).toBe(true);
    ...
    test('Harvest At 1/20 Second Epoch After Adding Rewards', async () => {
      ...
      const canHarvest = await checkCanHarvest(rewardDistState, BigInt(genesis) + epochDuration + epochDuration / 20n);
      expect(canHarvest.returns).toBe(true);
    ...
    test('Harvest No Reward Should Fail', async () => {
      ...
      const canHarvest = await checkCanHarvest(rewardDistState, BigInt(genesis) + (epochDuration / 1000n));
      expect(canHarvest.returns).toBe(false);
    ...
    test('Harvest Very Small Reward Should Fail', async () => {
      ...
      const canHarvest = await checkCanHarvest(rewardDistState, BigInt(genesis) + epochDuration + (epochDuration / 1000n));
      expect(canHarvest.returns).toBe(false);
    ...
    test('Harvest Before Adding Rewards', async () => {
      await expectAssertionError(checkCanHarvest(fixture.selfState, BigInt(genesis) + (epochDuration / 2n)), fixture.address, Number(RewardDistributor.consts.ErrorCodes.RewardsNotInitialized));
