...
import {
  ...
  createApadToken,
  createBurnAlph,
  createSaleFlatPriceV2
} from './fixtures'
import { expectAssertionError, randomContractAddress, randomContractId } from '@alephium/web3-test'
import { SaleFlatPriceAlphV2, SaleFlatPriceAlphV2Types } from '../../artifacts/ts'
...
describe('Sale Flat Price Contract Testing', () => {
  ...
  let fixtureRegularSale: ContractFixture<SaleFlatPriceAlphV2Types.Fields>
  let fixtureWLAndRegularSale: ContractFixture<SaleFlatPriceAlphV2Types.Fields>
  let fixtureVestedSale: ContractFixture<SaleFlatPriceAlphV2Types.Fields>
  ...
  beforeEach(async () => {
    ...
    fixtureRegularSale = createSaleFlatPriceV2(
    ...
    fixtureVestedSale = createSaleFlatPriceV2(
      rewardDistributor.contractId,
      seller,
      accountTemplate.contractId,
      4n * (10n ** 15n),
    ...
    fixtureWLAndRegularSale = createSaleFlatPriceV2(
      rewardDistributor.contractId,
      seller,
      accountTemplate.contractId,
      5n * (10n ** 14n),
  ...
  // SECTION: Helpers
  // --------------------
  function calculateTokensReceivedPerAlph(state: ContractState<SaleFlatPriceAlphV2Types.Fields>, alphAmount: bigint, existingContracts: ContractState[]) {
    return SaleFlatPriceAlphV2.tests.calculateSaleTokensReceivedPerBidTokens({
  ...
  function getBuyerAccount(state: ContractState<SaleFlatPriceAlphV2Types.Fields>, buyer: string, existingContracts: ContractState[]) {
    return SaleFlatPriceAlphV2.tests.getSubContractId({
      initialFields: state.fields,
      initialAsset: state.asset,
      address: state.address,
      existingContracts: existingContracts,
  ...
  async function calculateClaimableAmount(state: ContractState<SaleFlatPriceAlphV2Types.Fields>, buyer: string, timestamp: bigint, existingContracts: ContractState[]) {
    var bAccount = await getBuyerAccount(state, buyer, existingContracts);
    return await SaleFlatPriceAlphV2.tests.calculateClaimableAmount({
      initialFields: state.fields,
      initialAsset: state.asset,
      address: state.address,
      existingContracts: existingContracts,
  ...
  function buy(state: ContractState<SaleFlatPriceAlphV2Types.Fields>, buyer: string, amount: bigint, wlProof: string, timestamp: bigint, existingContracts: ContractState[]) {
    const inputAssets = [{ address: buyer, asset: { alphAmount: (DUST_AMOUNT * 100n) + ONE_ALPH + amount } }]
    return SaleFlatPriceAlphV2.tests.buy({
      initialFields: state.fields,
      initialAsset: state.asset,
      address: state.address,
      existingContracts: existingContracts,
  ...
  function claim(state: ContractState<SaleFlatPriceAlphV2Types.Fields>, claimer: string, amount: bigint, timestamp: bigint, existingContracts: ContractState[]) {
    const inputAssets = [{ address: claimer, asset: { alphAmount: (DUST_AMOUNT * 150n) } }]
    return SaleFlatPriceAlphV2.tests.claim({
      initialFields: state.fields,
      initialAsset: state.asset,
      address: state.address,
      existingContracts: existingContracts,
  ...
  function claimRefund(state: ContractState<SaleFlatPriceAlphV2Types.Fields>, claimer: string, amount: bigint, timestamp: bigint, existingContracts: ContractState[]) {
    const inputAssets = [{ address: claimer, asset: { alphAmount: (DUST_AMOUNT * 150n) } }]
    return SaleFlatPriceAlphV2.tests.claimRefund({
      initialFields: state.fields,
      initialAsset: state.asset,
      address: state.address,
      existingContracts: existingContracts,
  ...
  function updateMerkle(state: ContractState<SaleFlatPriceAlphV2Types.Fields>, caller: string, newMerkle: string, timestamp: bigint, existingContracts: ContractState[]) {
    const inputAssets = [{ address: caller, asset: { alphAmount: (DUST_AMOUNT * 150n) } }]
    return SaleFlatPriceAlphV2.tests.setMerkleRoot({
      initialFields: state.fields,
      initialAsset: state.asset,
      address: state.address,
      existingContracts: existingContracts,
  ...
  function updateSaleDate(state: ContractState<SaleFlatPriceAlphV2Types.Fields>, caller: string, newSaleStart: bigint, newSaleEnd: bigint, newWhitelistSaleStart: bigint, newWhitelistSaleEnd: bigint, timestamp: bigint, existingContracts: ContractState[]) {
    const inputAssets = [{ address: caller, asset: { alphAmount: (DUST_AMOUNT * 150n) } }]
    return SaleFlatPriceAlphV2.tests.setSaleDates({
      initialFields: state.fields,
      initialAsset: state.asset,
      address: state.address,
      existingContracts: existingContracts,
  ...
  function updateWLMAxBid(state: ContractState<SaleFlatPriceAlphV2Types.Fields>, caller: string, newWLMaxBid: bigint, timestamp: bigint, existingContracts: ContractState[]) {
    const inputAssets = [{ address: caller, asset: { alphAmount: (DUST_AMOUNT * 150n) } }]
    return SaleFlatPriceAlphV2.tests.setWhitelistBuyerMaxBid({
      initialFields: state.fields,
      initialAsset: state.asset,
      address: state.address,
      existingContracts: existingContracts,
  ...
  function accountExists(state: ContractState<SaleFlatPriceAlphV2Types.Fields>, address: string, existingContracts: ContractState[]) {
    return SaleFlatPriceAlphV2.tests.accountExists({
      initialFields: state.fields,
      initialAsset: state.asset,
      address: state.address,
      existingContracts: existingContracts,
  ...
  describe('Regular Sale Functionality', () => {
    ...
    test('Correctly calculates tokens received for various ALPH amounts on 0 decimal token with decimal price', async () => {
      ...
      const sale = createSaleFlatPriceV2(
        randomContractId(),
        seller,
        randomContractId(),
        7n * (10n ** 16n),
    ...
    test('Prevents buying tokens with more than 18 decimals', async () => {
      ...
      await expectAssertionError(calculateTokensReceivedPerAlph(sale.selfState, 10n * (10n ** 18n), sale.dependencies), sale.address, Number(SaleFlatPriceAlphV2.consts.ErrorCodes.SaleTokenMoreThan18Decimal));
    });

    test('Prevents buying outside sale dates and allows during sale dates', async () => {
      await expectAssertionError(buy(fixtureRegularSale.selfState, buyer1, 10n * (10n ** 18n), "", BigInt(genesis + 1), fixtureRegularSale.dependencies), fixtureRegularSale.address, Number(SaleFlatPriceAlphV2.consts.SaleBaseErrorCodes.SaleNotLive));
      await expectAssertionError(buy(fixtureRegularSale.selfState, buyer1, 10n * (10n ** 18n), "", BigInt(genesis + 1) + (86400000n * 3n), fixtureRegularSale.dependencies), fixtureRegularSale.address, Number(SaleFlatPriceAlphV2.consts.SaleBaseErrorCodes.SaleNotLive));
    ...
    test('Prevents claiming within sale period, allows after sale completion and target achievement', async () => {
      ...
      var result = await buy(fixtureRegularSale.selfState, buyer1, 10n * (10n ** 18n), "", BigInt(genesis + 1) + (86400000n * 1n), fixtureRegularSale.dependencies);
      var state = getContractState<SaleFlatPriceAlphV2Types.Fields>(result.contracts, fixtureRegularSale.contractId)

      await expectAssertionError(claim(state, buyer1, 10n * (10n ** 18n), BigInt(genesis + 1) + (86400000n * 2n), result.contracts), fixtureRegularSale.address, Number(SaleFlatPriceAlphV2.consts.SaleBaseErrorCodes.SaleNotFinished));
      await expectAssertionError(claim(state, buyer1, 10n * (10n ** 18n), BigInt(genesis + 1) + (86400000n * 3n), result.contracts), fixtureRegularSale.address, Number(SaleFlatPriceAlphV2.consts.SaleBaseErrorCodes.SaleClaimNotAvailable));
      ...
      state = getContractState<SaleFlatPriceAlphV2Types.Fields>(result.contracts, fixtureRegularSale.contractId)
    ...
    test('Prevents claiming refund within sale period, allows after sale completion and target not achievement', async () => {
      ...
      await expectAssertionError(claimRefund(state, buyer1, 10n * (10n ** 18n), BigInt(genesis + 1) + (86400000n * 2n), result.contracts), fixtureRegularSale.address, Number(SaleFlatPriceAlphV2.consts.SaleBaseErrorCodes.SaleNotFinished));
  ...
  describe('Vested Sale Functionality', () => {
    test('Amounts should correctly calculate for vesting', async () => {
      var result = await buy(fixtureVestedSale.selfState, buyer1, 150n * (10n ** 18n), "", BigInt(genesis + 1) + (86400000n * 1n), fixtureRegularSale.dependencies);
      var state = getContractState<SaleFlatPriceAlphV2Types.Fields>(result.contracts, fixtureVestedSale.contractId)

      result = await buy(state, buyer2, 75n * (10n ** 18n), "", BigInt(genesis + 1) + (86400000n * 1n), result.contracts);
      state = getContractState<SaleFlatPriceAlphV2Types.Fields>(result.contracts, fixtureVestedSale.contractId);

      result = await buy(state, buyer3, 50n * (10n ** 18n), "", BigInt(genesis + 1) + (86400000n * 1n), result.contracts);
      state = getContractState<SaleFlatPriceAlphV2Types.Fields>(result.contracts, fixtureVestedSale.contractId);

      var vestedAmount = await calculateClaimableAmount(state, buyer1, BigInt(genesis + 1) + (86400000n * 4n), result.contracts);
      expect(vestedAmount.returns).toBe(18750n * 10n ** 18n);

      var vestedAmount = await calculateClaimableAmount(state, buyer2, BigInt(genesis + 1) + (86400000n * 4n), result.contracts);
      expect(vestedAmount.returns).toBe(9375n * 10n ** 18n);
      ...
      var vestedAmount = await calculateClaimableAmount(state, buyer3, BigInt(genesis) + (86400000n * 94n), result.contracts);
      expect(vestedAmount.returns / 10n ** 18n).toBe(6250n + 1558n);

      var vestedAmount = await calculateClaimableAmount(state, buyer3, BigInt(genesis) + (86400000n * 183n), result.contracts);
      expect(vestedAmount.returns / 10n ** 18n).toBe(6250n + 3082n);

      var vestedAmount = await calculateClaimableAmount(state, buyer3, BigInt(genesis) + (86400000n * 368n), result.contracts);
      expect(vestedAmount.returns / 10n ** 18n).toBe(6250n * 2n);

      var vestedAmount = await calculateClaimableAmount(state, buyer3, BigInt(genesis) + (86400000n * 728n), result.contracts);
      expect(vestedAmount.returns / 10n ** 18n).toBe(6250n * 2n);
    });
    ...
    test('Amounts should correctly calculate for vesting after claims', async () => {
      ...
      await expectAssertionError(
        ...
        Number(SaleFlatPriceAlphV2.consts.ErrorCodes.InvalidClaimAmount)
      );
  ...
  describe('Assertions Functionality', () => {
    test("Should fail if the sale is not live", async () => {
      await expectAssertionError(
        ...
        Number(SaleFlatPriceAlphV2.consts.SaleBaseErrorCodes.SaleNotLive)
      );
    });
    test("Should fail if the sale owner attempts to buy", async () => {
      await expectAssertionError(
        ...
        fixtureRegularSale.address,
        Number(SaleFlatPriceAlphV2.consts.ErrorCodes.SaleOwnerCanNotBid)
      );
    });
    test("Should fail if the sale exceeds the total token amount", async () => {
      ...
      var state = getContractState<SaleFlatPriceAlphV2Types.Fields>(result.contracts, fixtureRegularSale.contractId)
      await expectAssertionError(
        buy(state, buyer1, 15000n * (10n ** 18n), "", BigInt(genesis + 1) + saleStartDelay, result.contracts),
        fixtureRegularSale.address,
        Number(SaleFlatPriceAlphV2.consts.ErrorCodes.SaleTokenTotalExceeded)
      );
    });
    test("Claiming tokens should fail if the sale is not finished", async () => {
      await expectAssertionError(
        ...
        fixtureRegularSale.address,
        Number(SaleFlatPriceAlphV2.consts.SaleBaseErrorCodes.SaleNotFinished)
      );
    });
    test("Claiming tokens should fail if the sale conditions for claiming are not met", async () => {
      await expectAssertionError(
        ...
        fixtureRegularSale.address,
        Number(SaleFlatPriceAlphV2.consts.SaleBaseErrorCodes.SaleClaimNotAvailable)
      );
    });
    test("Claiming refund should fail if the conditions for refund are not met", async () => {
      ...
      await expectAssertionError(
        ...
        fixtureRegularSale.address,
        Number(SaleFlatPriceAlphV2.consts.SaleBaseErrorCodes.SaleRefundNotAvailable)
      );
    });
    test("Setting Merkle Root should fail if the sale is not WL Sale", async () => {
      await expectAssertionError(
        ...
        fixtureRegularSale.address,
        Number(SaleFlatPriceAlphV2.consts.SaleBaseErrorCodes.SaleIsNotWLSale)
      );
    });
    test("Setting Merkle Root should fail if the merkle root is not the correct size", async () => {
      await expectAssertionError(
        ...
        Number(SaleFlatPriceAlphV2.consts.SaleBaseErrorCodes.InvalidMerkleRoot)
      );
    });
    ...
    test("Setting Merkle Root should fail if the sale has already started", async () => {
      await expectAssertionError(
        ...
        fixtureWLAndRegularSale.address,
        Number(SaleFlatPriceAlphV2.consts.SaleBaseErrorCodes.SaleCanNotBeEditedAtThisTime)
      );
    });
    test("Setting Merkle Root should fail if caller is not owner", async () => {
      await expectAssertionError(
        ...
        fixtureWLAndRegularSale.address,
        Number(SaleFlatPriceAlphV2.consts.SaleBaseErrorCodes.SaleUpdateUnauthorized)
      );
    });
    ...
    test("Setting WL Max Bid should fail if value is out of range", async () => {
      await expectAssertionError(
        ...
        fixtureWLAndRegularSale.address,
        Number(SaleFlatPriceAlphV2.consts.ErrorCodes.BidLessThanMin)
      );
      await expectAssertionError(
        ...
        fixtureWLAndRegularSale.address,
        Number(SaleFlatPriceAlphV2.consts.ErrorCodes.BidMoreThanMax)
      );
    });
    ...
    test("Setting Sale Dates should fail if value is out of range", async () => {
      await expectAssertionError(
        updateSaleDate(fixtureWLAndRegularSale.selfState, seller,
          ...
          BigInt(genesis + (86300000 - 1800000)), fixtureWLAndRegularSale.dependencies),
        fixtureWLAndRegularSale.address,
        Number(SaleFlatPriceAlphV2.consts.SaleBaseErrorCodes.WLSaleStartMustBeWithinSaleDates)
      );
      await expectAssertionError(
        updateSaleDate(fixtureWLAndRegularSale.selfState, seller,
          BigInt(genesis + (86300000 + 1800000)),
          ...
          BigInt(genesis + (86300000 - 1800000)), fixtureWLAndRegularSale.dependencies),
        fixtureWLAndRegularSale.address,
        Number(SaleFlatPriceAlphV2.consts.SaleBaseErrorCodes.WLSaleEndMustBeWithinSaleDatesAndAfterWLSaleStart)
      );
      await expectAssertionError(
        updateSaleDate(fixtureWLAndRegularSale.selfState, seller,
          BigInt(genesis + (86300000 + 1800000)),
  ...
  describe('Whitelist (WL) Sale Functionality', () => {
    test('Allows WL buyers to bid within the WL period', async () => {
      ...
      var state = getContractState<SaleFlatPriceAlphV2Types.Fields>(result.contracts, fixtureWLAndRegularSale.contractId)
    ...
    test('Allows WL buyers to Check Eligibility', async () => {
      const result = await SaleFlatPriceAlphV2.tests.checkIsWhitelisted({
        testArgs: {
      ...
      const result2 = await SaleFlatPriceAlphV2.tests.checkIsWhitelisted({
        testArgs: {
          account: wlBuyers[0].address,
    ...
    test('Prevents WL buyers from bidding more than max', async () => {
      ...
      var state = getContractState<SaleFlatPriceAlphV2Types.Fields>(result.contracts, fixtureWLAndRegularSale.contractId)
      await expectAssertionError(buy(state, wlBuyers[0].address, 10000n * (10n ** 18n), wlBuyers[0].proof, BigInt(genesis + 1) + (86400000n * 1n), result.contracts), fixtureWLAndRegularSale.address, Number(SaleFlatPriceAlphV2.consts.ErrorCodes.BidMoreThanMax));
    });

    test('Prevents non-WL buyers from bidding during the WL period', async () => {
      await expectAssertionError(buy(fixtureWLAndRegularSale.selfState, buyer2, 10n * ONE_ALPH, wlBuyers[0].proof, BigInt(genesis + 1) + (86400000n * 1n), fixtureWLAndRegularSale.dependencies), fixtureWLAndRegularSale.address, Number(SaleFlatPriceAlphV2.consts.ErrorCodes.BuyerNotWhitelisted));
    });
